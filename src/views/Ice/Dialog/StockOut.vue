<template>
  <v-dialog v-model="dialog" persistent max-width="800px">
    <v-card>
      <v-card-title> 整冰制冰出库 </v-card-title>
      <v-card-text>
        <v-form ref="form" v-model="valid" lazy-validation>
          <v-row dense>
            <v-col cols="6" md="6" sm="6">
              <v-menu v-model="stockInTimeMenu" :close-on-content-click="false" :nudge-right="40" transition="scale-transition" offset-y min-width="290px">
                <template v-slot:activator="{ on }">
                  <v-text-field v-model="iceStockInfo.stockTime" label="出库时间" prepend-icon="event" readonly v-on="on"></v-text-field>
                </template>
            ...
            <v-col cols="6" md="6" sm="6">
              <v-text-field label="出库数量*" v-model="iceStockInfo.flowCount" :rules="digitalRules" required></v-text-field>
            </v-col>
  ...
  methods: {
    ...
    submit() {
      if (this.$refs.form.validate()) {
        ...
        ice.stockOut(this.iceStockInfo).then((res) => {
          if (res.status == 0) {
            vm.$store.commit('alertSuccess', '冰块出库成功')
            vm.$emit('close')
