...
import StoreFind from '../Mod/StoreFind'
...
export default {
  ...
  data: () => ({
    ...
    stockOutTaskInfo: {},
    storeListData: [],
    sPositionInfo: null, //选中仓位
    sStoreInfo: null, // 选中库存
    positionStores: [], //选中仓位上的库存
  ...
  methods: {
    init(stockOutTaskId) {
      this.loadStockOutTask(stockOutTaskId)

      this.storeListData = []
      this.sPositionInfo = null
      this.sStoreInfo = null
      this.positionStores = []
      this.taskInfoList = []
    ...
    async loadStockOutTask(stockOutTaskId) {
    ...
    searchStore() {
      let vm = this
      store.findByStockOut({ stockOutId: this.stockOutTaskInfo.stockOutId, cargoId: this.stockOutTaskInfo.cargoId }).then(res => {
    ...
    editItem(item) {
      ...
      if (this.sPositionInfo.viceShelfCode) {
        this.editShelfCodes = [this.sPositionInfo.shelfCode, this.sPositionInfo.viceShelfCode]
      } else {
        this.editShelfCodes = [this.sPositionInfo.shelfCode]
      }
